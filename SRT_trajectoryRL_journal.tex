...
\author{
 Roy de Kleijn$^{1}$, George Kachergis$^{2}$, \& Bernhard Hommel$^{1}$
}
\affiliation{
...
\abstract{Sequential action makes up the bulk of human daily activity, and yet much remains unknown about how people learn such actions. %how people learn such actions is thus of great interest. 
In one motor learning paradigm, the serial reaction time (SRT) task, people are taught a consistent sequence of button presses by cueing them with the next target response. 
However, the SRT task only records keypress response times to a cued target, and thus cannot reveal the full time-course of motion, including predictive movements. 
This paper introduces a mouse movement trajectory SRT task in which the cursor must be moved to a cued location. 
We replicate keypress SRT results, but also find that predictive movement--before the next cue appears--increases during the experiment. 
Moreover, trajectory analyses reveal that people develop a centering strategy under uncertainty. In a second experiment we make prediction explicit, no longer cueing targets. 
Thus, participants must explore the response alternatives and learn via reinforcement, receiving rewards and penalties for correct and incorrect actions, respectively. 
Participants were not told whether the sequence of stimuli was deterministic, nor if it would repeat, nor how long it was. Given the difficulty of the task, it is unsurprising that some learners performed poorly. However, many learners performed remarkably well, and some acquired the full 10-item sequence within 10 repetitions. Comparing the high- and low-performers' detailed results in this reinforcement learning (RL) task with the first experiment's cued trajectory SRT task, we find both similarities and discrepancies. Finally, we found that humans in this task outperform three standard model-free RL models.}
...
%, a topic of renewed interest as hybrid deep learning and hierarchical RL models have recently outperformed humans in popular board games (Go) and computer games (ATARI). 
% we should cite a couple recent RL sequence learning papers (even deep learning; like DeepMind Atari paper), maybe: Dezfouli:2012 - https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3325518/
% show that it's an open problem of recent interest!
...
Data were analyzed from the 22 participants (11 per condition) that completed the experiment. Median movement time to a target was 1,040 ms (sd: 1,776). Of 17,578 target arrival times, 84 were removed for being slower than 2,816 ms (median+sd). Each subject's median RT for correct movements on each block was computed. Figure~\ref{fig:RT-ACC-exp1}a shows the mean of median RTs by block for the two conditions. Participants in both conditions got faster over the course of the experiment, but participants in the NB87 sequence condition improved more than those in the random condition, replicating the \citeA{Nissen:1987} speedup. There was a 25\% reduction in reaction time over the course of training. These data were analyzed by a two-way analysis of variance, which indicated significant main effects of condition ($F$(1,20) = 31.3, \textit{p} \textless .001) and block ($F$(7,168) = 6.3, \textit{p} \textless .05), and a significant interaction effect ($F$(7,210) = 14.7, \textit{p} \textless .01) between the two.
...
\begin{figure}[t] % [h]
    \caption{Experiment 1 RTs and error rates by block.}
        \centering
        \begin{subfigure}[t]{0.41\textwidth}
                \includegraphics[width=\textwidth]{figures/exp1_basic_RT_result}
                \subcaption{Mean of median RTs by block show that both conditions sped up over the course of Experiment 1, but that NB87 improved more. Error bars show +/-1SE.}
        \end{subfigure}
        ~
        \begin{subfigure}[t]{0.41\textwidth} 
                \includegraphics[width=\textwidth]{figures/exp1_training_mistakes_per_block}
                \subcaption{Mean number of errors by block shows only the NB87 participants made an increasing number of errors. Error bars show +/-1SE.}
        \end{subfigure}
        \label{fig:RT-ACC-exp1}
\end{figure}
...
%\end{figure} 
% GK: this is a really big graph to just say "errors went  up over time for NB87, but not random condition" I feel like a smaller version (combined with another figure? like the RT one?) would be sufficient...show something good and bad at the same time? or maybe we could even show a scatterplot with each subject's speedup vs. error rate (last block - first block), with plot character showing their condition?
% oh, or maybe it's good to make a correct vs. incorrect RT RT graph, like we did for Exp 2? (Fig 8) dot size could show proportion of responses.
...
\begin{figure}[t] % [h]
    \caption{RTs and error rates by median split and sequential position.}
        \centering
        \begin{subfigure}[t]{0.41\textwidth}
                \includegraphics[width=\textwidth]{figures/RL_HitRT_by_seqpos}
                \subcaption{Mean of subjects' median correct response times by median split and sequential position. The correct RTs for the two performance groups were not significantly correlated ($r=0.17$, t(8)=0.48, $p=0.65$). Error bars reflect +/-1SE.}
        \end{subfigure}
        ~
        \begin{subfigure}[t]{0.41\textwidth} 
                \includegraphics[width=\textwidth]{figures/RL_mistakes_by_seqpos}
                \subcaption{The mean number of errors made at each position in the sequence split by performance group. The errors are highly correlated ($r=$.79, t(8)=3.68, $p<.01$), though note how much worse sequence position 5 was for the low-performing group relative to the next-worst position (8). Low-performers showed twice as many errors in position 5 as in 8, while the high-performing group showed only a 25\% increase in errors. Error bars reflect +/-1SE.}
        \end{subfigure}
